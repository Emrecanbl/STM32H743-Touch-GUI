...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  /* USER CODE BEGIN 6 */
   ...
   //Data Format Vcp_Data_Available = &240&20&4800&35
   //Voltage&Ampere&Power&Temparature
   CDC_Transmit_FS("Data Received\n", strlen("Data Received\n"));
   ...
   while (token != NULL){
	...
	sprintf(Elek,"%dV/%dA", Data_recv[0],Data_recv[1]);
	sprintf(Power,"%dW", Data_recv[2]);
	sprintf(Temp,"%dC", Data_recv[3]);
	lv_label_set_text(ui_Label7, Temp);
	lv_label_set_text(ui_Label6, Power);
	lv_label_set_text(ui_Label5, Elek);
   return (USBD_OK);
  /* USER CODE END 6 */
}
